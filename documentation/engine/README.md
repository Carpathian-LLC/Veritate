...
| v9 | `VERITATE_MODEL_VERSION_BOOST` | INT8 per col | per col | adds `act_boost` (residual scale 1/2/4). default for `export_checkpoint` |
| v10 | `VERITATE_MODEL_VERSION_TERNARY` | ternary (BitNet b1.58, 5 trits/byte) | per-tensor `gamma_q24` | 5× smaller `.bin`. weights decode to `{-1, 0, +1}`-valued INT8 at load time; same hot path as INT8 |
