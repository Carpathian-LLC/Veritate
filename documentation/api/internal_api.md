...
| method + path | def | params | response | purpose |
|---|---|---|---|---|
| `GET /generate` | `backends_routes.py:716` | `prompt:str`, `temperature:float=0.7`, `top_k:int=40` (clamped 1..256), `max_new:int=200` (capped at 4096), `backend:str=c`, `ablate_layer:int=-1`, `ablate_neuron:int=-1`, `addons:str` (csv), `fast:str` (`kv`\|`mtp`\|`mtp-verify`\|`adaptive`), `constrained:str`, `adaptive_threshold:float=0.8`, `rep_window:int=0`, `rep_penalty:float=0`, `no_repeat_ngram:int=0` (byte-level repetition control; all zero = off; chat mode sends 256 / 0.5 / 16), `rag:str` (corpus path, **loopback only**), `rag_k:int=3` (max 16), `rag_compress:str` (`off`\|`crude`\|`word_ppl[:keep_frac]`) | **SSE.** First a `meta` frame (`checkpoint, n_params, layers, heads, ffn, vocab, seq, hidden, has_memory, prompt, prompt_bytes, backend, c_exe, c_model, c_model_dir, c_model_path`); an optional `rag` frame (`backend, corpus, top_k, hits, prefix_bytes, prefix_text, compress`); then `kind:"token"` frames (`byte, argmax_byte, T, fwd_ms, entropy_bits, surprise_bits, ffn_full, ffn_top, ffn_argmax, ffn_downsample, decisiveness, dla_picked, dla_argmax, dla_cand, ablation, margin, entropy, lens_consistency, residual_stab, confidence, attn, info_flow, res, contrib, lens, cand, memory, backend`); ends with `kind:"stop"` (`reason`) then `done`. A non-numeric `temperature`/`top_k`/`max_new`/`ablate_*` returns a `kind:"error"` SSE frame + `done` (not a JSON 500), so the EventSource client stays parseable. `400` invalid `fast`/`rag_k`/`rag_compress`/missing corpus, `403` `rag=` from a non-loopback client, `500` stream error | byte-by-byte generation with full interpretability telemetry, optional ablation, RAG prefix, decode addons, and constrained decoding |
| `GET /agent/stream` | `backends_routes.py:914` | `prompt:str` (required), `max_turns:int=6` (max 16), `best_of_n:int=1` (max 8), `temperature:float=0.7`, `top_k:int=40`, `seed:int=0`, `corpus:str` (path, **loopback only**), `fs_root:str` (path, **loopback only**), `tools:str` (csv) | **SSE.** `agent_meta` frame (`tools, max_turns, best_of_n`), then per-turn agent frames, then `done`. `400` missing prompt / bad path / no usable tools, `403` `corpus=`/`fs_root=` from a non-loopback client | stream a full agent trace (tool calls + reasoning) |
...
<<<<<<< Updated upstream
| `POST /hybrid/chat` | `hybrid_routes.py:830` | body `message:str` (required), `model`, `backend` (`pytorch`\|`c`), `use_rag:bool`, `use_logs:bool`, `kb_scope` (`all`\|`platform`\|`user`), `k:int`, `history:[{role, content}]`, `summary:str`, `mri:bool` (opt-in per-byte MRI telemetry, local models only) | `{ok, answer, model, backend, confident:bool, sources:[{text, score}], memory:{summary, turns}, context:{turns, chars, char_limit, pct}}`; with `mri:true` on a local model an extra top-level `mri:[frame,…]` carries the per-byte frames (`kind:"meta"`, one `kind:"token"` per byte, `kind:"stop"`) — the same objects `/generate` emits, unfiltered; absent/false = today's payload byte-for-byte. `400` empty message, `503` model/provider unavailable, `500` error | conversational chat with RAG + teacher fallback + memory |
| `POST /hybrid/chat/stream` | `hybrid_routes.py:845` | same body as `/hybrid/chat` (incl. `mri:bool`) | `text/event-stream`: `{kind:"delta", text}` frames as the model generates (true per-token for local models on the selected `backend`, word-chunked for remote), then one `{kind:"done", answer, model, backend, confident, sources, memory, context}` frame; a mid-stream failure is a `{kind:"error", error}` frame (status already 200). With `mri:true` on a local model the per-byte frames interleave as their own `kind:"meta"`/`kind:"token"`/`kind:"stop"` frames alongside the `delta` frames (the `done` frame does not repeat them). `400` empty message | streaming twin of `/hybrid/chat` the chat pages consume |
| `POST /v1/chat/completions` | `hybrid_routes.py:627` | body `model:str` (local name or `cloud`), `messages:[{role, content}]` (required, ends in user), `stream:bool`, `temperature?`, `max_tokens?`, `top_k?` (accepted, not plumbed), `mri:bool` (opt-in per-byte MRI telemetry, local models only) | non-stream: OpenAI `{id, object:"chat.completion", created, model, choices:[{index, message:{role, content}, finish_reason:"stop"}], usage}`; stream: `text/event-stream` of `chat.completion.chunk` frames (role frame, content-delta frames, a `finish_reason:"stop"` frame) then `data: [DONE]`. **Local trained models stream true per-token as generated** (the same generator `/generate` uses); cloud/teacher chunk the buffered answer word-by-word. With `mri:true` on a local model: non-stream adds a top-level `mri:[frame,…]` frame list; stream interleaves each frame as a `chat.completion.chunk` with empty `delta` carrying the frame under a top-level `mri` key (off-the-shelf OpenAI clients ignore the empty-delta chunk and unknown key). Frames are the unfiltered `/generate` objects; absent/false = today's behavior. See [external_api.md](external_api.md) for the frame shape and size caveat. `400` bad body, `503` model/provider unavailable, `500` error | OpenAI-compatible chat completions; same model routing as `/hybrid/chat` |
=======
...
| `POST /hybrid/chat/stream` | `hybrid_routes.py:845` | same body as `/hybrid/chat` | `text/event-stream`: `{kind:"delta", text}` frames as the model generates (true per-token for local models on the selected `backend`, word-chunked for remote), then one `{kind:"done", answer, model, backend, confident, sources, memory, context}` frame; a mid-stream failure is a `{kind:"error", error}` frame (status already 200). `400` empty message | streaming twin of `/hybrid/chat` the chat pages consume |
| `POST /v1/chat/completions` | `hybrid_routes.py:627` | body `model:str` (local name or `cloud`), `messages:[{role, content}]` (required, ends in user), `stream:bool`, `temperature?`, `max_tokens?`, `top_k?` (accepted, not plumbed) | non-stream: OpenAI `{id, object:"chat.completion", created, model, choices:[{index, message:{role, content}, finish_reason:"stop"}], usage}`; stream: `text/event-stream` of `chat.completion.chunk` frames (role frame, content-delta frames, a `finish_reason:"stop"` frame) then `data: [DONE]`. **Local trained models stream true per-token as generated** (the same generator `/generate` uses); cloud/teacher chunk the buffered answer word-by-word. `400` bad body, `503` model/provider unavailable, `500` error | OpenAI-compatible chat completions; same model routing as `/hybrid/chat` |
| `POST /v1/chat/mri` | `hybrid_routes.py:934` | same body as `/v1/chat/completions`; `stream` defaults to `true` | OpenAI-shaped sibling that also emits per-byte MRI telemetry (local trained models only). Stream: `text/event-stream` of `chat.completion.chunk` frames (role frame, then one chunk per MRI frame carrying that frame under a top-level `mri` key plus the byte's text delta in `choices[0].delta.content`, a `finish_reason:"stop"` frame) then `data: [DONE]`. Non-stream: the OpenAI `chat.completion` plus the full frame list under `mri`. `400` bad body or a non-local (cloud/teacher) model with no MRI, `503` model unavailable | reply text + model-internals frames in one OpenAI stream |
>>>>>>> Stashed changes

## hallucination

Answer analysis for the hallucination overlay. Generates an answer through the same backends as `/generate`, then scores it: per-byte confidence rolled up to word/sentence/paragraph spans (byte-exact offsets), grounded overlap against the retrieved context, context divergence (answer with vs without context), and a training-data provenance proxy (nearest BM25 passages over a bounded sample of `trainers/corpus`, similarity only, not causal attribution).

| method + path | def | params | response | purpose |
|---|---|---|---|---|
| `POST /hallucination/analyze` | `hallucination_routes.py` | body `model:str` (required), `backend` (`pytorch`\|`c`), `prompt:str` (raw generation) **or** `message:str` (chat, triggers RAG framing), `use_rag:bool`, `kb_scope` (`all`\|`platform`\|`user`), `k:int`, `max_new:int`, `temperature:float`, `frames:[{byte,confidence,surprise_bits,entropy_bits}]` (deferred mode: grade the client-streamed answer directly, no re-generation, `context_divergence:null`) | `{ok, answer, overall:{confidence, hallucination_risk, grounded_fraction\|null, context_divergence\|null, uncertain:bool, verdict}, spans:{paragraphs:[{text,start,end,confidence,confidence_min,grounded,sentences:[{...,words:[{text,start,end,confidence,surprise_bits,entropy_bits,grounded}]}]}]}, provenance:{grounded_sources:[{span,source_chunk,score}], training_matches:[{span,passages:[{text,corpus,score}]}]}, context:[{text,score}], confidence_source}`; `400` missing model/prompt, `503` backend unavailable | grade an answer for hallucination risk with per-span confidence + grounding + provenance |

`verdict` (context present unless noted) is one of: `refused` (answer is an "I don't know"); `grounded` (grounded_fraction >= 0.85); `likely_hallucinated` (context_divergence >= 0.8 and grounded_fraction < 0.85, i.e. the answer mixes context tokens with unsupported ones and diverges hard from the no-context answer, whether grounded_fraction is 0.0 or 0.5); `partially_grounded` (0.15 < grounded_fraction < 0.85 with lower divergence, the "check this" state); `low_confidence` (overall confidence under the calibration threshold); or `ungrounded_ok`. `grounded_fraction` and `context_divergence` are `null` when no context was retrieved. `confidence_source` is `byte_confidence` (the calibrated per-byte score both backends emit) or `entropy_surprise` (derived fallback when a frame carries no confidence).
...
| `POST /extensions/install` | `extensions_routes.py:31` | body `id:str` (required) | `{ok, extension:{id, installed:true}}`; `400` `id` missing, `404` no source | clear the disabled flag + copy `canonical/<id>` → `installed/<id>`; live, no restart |
| `POST /extensions/uninstall` | `extensions_routes.py:42` | body `id:str` (required) | `{ok}`; `400` `id` missing | disable the id in `disabled.json` (a `before_request` gate 404s its routes live) + remove `installed/<id>` code, keeping `data/` |
