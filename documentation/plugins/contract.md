...
| `sizes` | object | shape table for the size dropdown. Keys are size labels (`"80m"`, `"1b3"`); values are shape dicts with `layers`, `hidden`, `ffn`, `heads`, `params`, optionally `active_params` (MoE). The dashboard's size dropdown choices and the VRAM estimator both read from this; the plugin's `SIZE_PRESETS` is loaded from here. Required for trainers that expose `size` as a form field. |
...
|---|---|---|---|
| `size` | string | `"120m"` | shape preset name; must be a key in the manifest's top-level `sizes` table. |
...
### example: ternary + MoE trainer (MEGA-style)
