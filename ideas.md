...
2. **Engine top-K=2 refused (MEDIUM, ~1 day).** `veritate_engine/src/model.c` v11 loader refuses `router_topk > 1`. Trainer uses top_k=2. Options: run bench in PyTorch only (no engine numbers), or set MOE_TOP_K=1 (loses the "fine-grained" quality claim).
...
Wall-clock: only the boundary-ranking op changes (a top-k over ~seq/2 candidates), so per-step FLOPs are UNCHANGED at same S. The win must come from val, not throughput; the falsifier's "at equal val" clause is the honest check. Rule 34d test surface: `tests/model/test_patched_dynamic_slots.py` — (a) fixed-shape invariance across all three entropy modes; (b) E-static is deterministic byte-for-byte across arches (macOS arm64 / Linux x86 / Windows x86); (c) fallback to first-S ordering when entropy-mode is off is BYTE-IDENTICAL to current `VeritatePatched`. Kill line: no entropy variant beats fixed-patch by >5 pts val on matched 10M, second seed per agent_roe. Retry condition if killed: try at 80M (small-scale ranking noise may swamp signal; BLT's own gains were at ≥400M). **Sequencing (2026-07-26): run T4 first.** T4 measures whether fixed patching's advantage holds at 121.8M, which sets both the size of the prize here and the scale this A/B should start at; if T4 shows the advantage growing, skip the matched-10M A/B entirely and open T2 at 80M.

### T4: the patching scale curve (measure the slope before funding T2)

Status: OPEN, **run before T2**. Opened 2026-07-26.

Every headline in `composed_efficiency_stack.md` is measured at exactly one size and one seed: Muon 1.60x, patching 1.82x, hybrid 0.9707, composed ~2.9x, all at the canonical 10M shape. The scale-up work (`successes.md` 2026-07-09) scaled the WINNING STACK (121.8M vs 270M, matched-token) and never re-ran the ablation, so **no dense-attention control exists above 10M and no lever has a slope.** The field's default prior on efficient-attention variants is that they win at small scale and vanish at large scale; a single-scale number does not survive that prior, and this is the gap most likely to get the paper dismissed. It is also the gap T2 needs closed: if patching itself decays with scale, the entropy variants are a smaller prize than they look.

**Mechanism: two matched arms at the `veritate_80m` 121.8M shape.** Dense (default trunk, `veritate_core/model.py`) vs `trunk=patched`, both Muon, both bf16, same corpus / step count / seed / precision, dashboard-launched, same single-variable discipline as the 10M race. Do NOT reuse the chat80m lineage as the patched arm: it was trained on a 6-corpus mix across pretrain/anneal/SFT phases, so comparing it to a clean fineweb_edu arm is confounded. Both arms must be fresh. Budget from the two numbers already measured: chat80m's ~16k tok/s at this shape sets the rate, the 10M race's 12000-step protocol sets the length; compute the estimate at launch rather than trusting a figure written here.

Two points is a slope. That alone converts every claim in the paper from "measured once" to "measured at 10M and 121.8M, advantage held / grew / decayed." A third point at 270M would be a curve, but it is not needed to close the gap.

**Decision lines (this is a MEASUREMENT, so the falsifier is on the interpretation, not on survival). Pre-registered before the run:**
- Patched wall-clock advantage at 121.8M **>= 1.82x**: the lever holds or grows with scale. Report the slope, and fund T2 directly at 80M, skipping the matched-10M A/B whose ranking noise T2's own retry condition already flags.
- **1.2x to 1.82x**: measured decay. Still publishable, but `composed_efficiency_stack.md` must restate its headline as a 10M result with a measured decay rather than a general 2.9x.
- **Below 1.2x**: the composed-stack headline does not survive contact with scale, and the paper needs a correction section. Per the research README's evidence discipline, a corrected finding is itself a result, so this outcome is reportable rather than a loss.

**Second-seed debt, closeable in the same batch.** `composed_efficiency_stack.md` already concedes that the hybrid-over-patched margin (0.007 bpb, 1.15x) sits inside the 5 percent band and carries a needs-a-second-seed caveat per agent_roe. Re-running E5 hybrid at 10M on a second seed is hours of compute and should ride along with this batch.

**BLOCKER, clear this first, independent of the experiment.** `composed_efficiency_stack.md`'s evidence trail cites `models/e1adamw_10m_qat/`, `e1muon_10m_qat/`, `e2patched_10m_qat/`, `e3recur_10m_qat/`, `e5hybrid_10m_qat/`. None of them exist on disk, and `/models/` is gitignored, so every `train.csv` behind the headline numbers is gone and the "ahead at 115 of 120 matched evals" claims are currently unverifiable. Curves are ~64 KB each (chat200m's is 64 KB). Add a `!models/**/train.csv` exception to `.gitignore` and commit whatever survives before any new run touches the tree. If the 10M curves are unrecoverable, the second-seed re-runs above are the recovery path and must dump to a committed location.
