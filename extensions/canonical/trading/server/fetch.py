...
# ------------------------------------------------------------------------------------
# Notes:
# - On-demand crypto backfill for the market platform. When data/crypto/<symbol>.csv
#   is missing, fetch the needed 1m bars from Binance and cache them in the schema
#   data.py reads, so a fresh install's market page works with zero manual data.
...
# extensions/canonical/trading/server/fetch.py
# ------------------------------------------------------------------------------------
# Imports:
...
TIMEOUT = 20
# Default fetchable majors so a fresh install (empty data dir) still lists instruments.
...
def main(argv=None):
    ...
    data/<source>/<SYMBOL>.csv, for building an order-flow training corpus.
    Existing files are left untouched.
    Run: python extensions/canonical/trading/server/fetch.py BTCUSDT ETHUSDT --bars 400000"""
    import argparse
    ...
    ap.add_argument("--source", default="crypto_extra", help="data dir subdir to write into")
    ap.add_argument("--bars", type=int, default=400000, help="1m bars per symbol to pull")
    args = ap.parse_args(argv)
    root = os.path.normpath(os.path.join(HERE, "..", "..", "..", "installed", "trading", "data", "market", args.source))
