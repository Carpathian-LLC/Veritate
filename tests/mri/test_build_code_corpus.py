...
# tests/mri/test_build_code_corpus.py
# ------------------------------------------------------------------------------------
# Imports:
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "..", "veritate_mri", "tools"))
...
def test_build_deterministic(tmp_path):
    ...
    for tag in ("a", "b"):
        ...
        outs.append((open(tr, "rb").read(), open(va, "rb").read()))
...
def test_build_emits_eot_separators(tmp_path):
    ...
    data = open(tr, "rb").read()
    assert data.count(b"<|endoftext|>") > 1
