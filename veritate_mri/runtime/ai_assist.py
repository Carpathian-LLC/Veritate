...
# veritate_mri/runtime/ai_assist.py
# ------------------------------------------------------------------------------------
# Imports:
...
OUTPUT_RULES = (
    ...
    "No intro, transition, or recap lines. Do not write 'Here are', 'Based on', 'In summary', or similar. "
    "Go straight from takeaway to bullets.\n"
    "Every bullet MUST cite at least one concrete number from the data: "
    "a step, a loss value, a percent delta, an lr, a grad norm, a tok/s. Keep each bullet under 22 words.\n"
    ...
    "Final line starts with 'next:' and gives one concrete action "
    "(keep training, lower LR by Nx, raise val frequency, restart from step N, etc.).\n"
...
PROMPT_LOSS_CURVE = (
    ...
    "You see a downsampled train series, the recent val series, a gap series (val minus train at matching steps), "
    "and the dashboard plateau verdict.\n"
    "Honor these facts:\n"
    ...
    "- The train series is DOWNSAMPLED. A delta on a train line is the delta to the prior LISTED train step "
    "in this block, not the prior actual training step. When you cite a train delta, name both endpoints "
    "(e.g., 'down 22% from step 360 to step 680'). Never write 'previous step' for a train delta.\n"
    "- Bullets must be in ascending step order.\n"
    "- Treat absolute changes under 1% as noise. Do not flag them.\n"
    "- Discuss gap trends only by citing entries from the gap series. "
    "Never invent a gap value for a step that is not in the gap series.\n"
    "- The 'next:' action must respect the schedule. If lr is already decaying via the schedule and the "
    "verdict is IMPROVING, SLOWING, or WARMING, do NOT propose lowering lr by hand. Recommend continuing "
    "to a concrete milestone step from the schedule. Manual lr changes are only valid when the verdict "
    "is REGRESSING or BOUNCING.\n"
    "- The takeaway line must include at least one number (a step, a loss, a percent, a slope). "
    "No vague openers like 'oscillations around a stable minimum'.\n"
    f"\n{OUTPUT_RULES}"
)
...
PROMPT_TRAIN_HEALTH = (
    ...
    "- States and what they mean: IMPROVING (slope clearly negative, keep going), SLOWING (still dropping "
    "but gently, normal late-training), PLATEAU (flat across recent evals, diminishing returns), BOUNCING "
    "(no clear direction, often a local minimum), REGRESSING (val rising on average, lower LR or restart "
    "from last good checkpoint), WARMING UP (under 4 val points, wait).\n"
...
def _post_messages(endpoint, key, messages):
    ...
    # Name the model, not just the URL: a hosted turn and a local turn otherwise read
    # identically in the log, and a hosted answer looks like a local model's output.
    logmod.info(LOG_SOURCE, f"POST {endpoint} model={MODEL_FIELD} hosted=yes "
                            f"bytes={len(req_body)} msgs={len(messages)} content_chars={total_chars}")
    ...
    if not choices:
        ...
        logmod.warn(LOG_SOURCE, f"no choices in response. keys={list(data.keys())} "
                                f"detail={_snip(str(finish))} raw={_snip(raw)}")
        return ""
    msg = choices[0].get("message") or {}
    finish_reason = choices[0].get("finish_reason")
    content = (msg.get("content") or "").strip()
    if not content:
        logmod.warn(LOG_SOURCE, f"empty content. finish_reason={finish_reason} "
                                f"message_keys={list(msg.keys())} raw={_snip(raw)}")
    else:
...
def _build_train_health(payload):
    ...
    user_message = (
        ...
        + ("Recent val measurements:\n" + "\n".join(f"- {ln}" for ln in pt_lines)
           if pt_lines else "Recent val measurements: none provided")
...
def _gap_series(name, val_pts, max_n, step_tol):
    ...
    for pt in (val_pts or [])[-max_n:]:
        ...
        if tl is None:
            cands = [k for k in train_by_step if abs(k - s_int) <= step_tol]
            if cands:
...
def _build_loss_curve(payload):
    ...
    user_message = (
        ...
        + ("Gap series (val minus train at matching steps):\n" + "\n".join(f"- {ln}" for ln in gap_lines)
           if gap_lines else "Gap series: none provided")
