...
Each pair is (correct_sentence, mutated_sentence). The probe scores both
under the model and counts a preference for the correct one when its
total negative log-likelihood is lower (mean ppl is lower).

Four pair-types, 50 each, hand-templated with slot fills. Output written
to veritate_mri/grade_eval/grammar/<type>.jsonl as
{"correct", "incorrect", "type"}.

Types:
    sv_agreement   subject-verb number agreement
    articles       a / an / the choice
    tense          tense consistency across a single sentence
    word_order     correct constituent order

Usage:
    python veritate_mri/tools/build_grammar_eval.py
"""
