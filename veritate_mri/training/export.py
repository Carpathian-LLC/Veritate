...
def moe_config(name):
    """returns (n_experts, router_topk, quant_mode_int) for a MoE checkpoint, or
    (1, 1, VERITATE_QUANT_INT8) for a dense one. A config opts into MoE by writing
    a `mega: {n_experts, router_topk}` block (or the legacy top-level `n_experts`).
    ...
    # n_experts > 1 (legacy MoE plugins). Treat as dense otherwise.
    is_moe = (int(mega.get("n_experts", 0)) > 1) or (int(cfg.get("n_experts", 0)) > 1)
    if not is_moe:
        return 1, 1, VERITATE_QUANT_INT8
    n_experts   = int(mega.get("n_experts",   cfg.get("n_experts",   1)))
    router_topk = int(mega.get("router_topk", cfg.get("router_topk", 1)))
...
def export_checkpoint(name, step):
    ...
    # RoPE: unsupported (binary encodes positions as pos_emb).
    # MTP:  exported as v12 (byte-0 head + untied lm_head).
    # Canonical trunks stay on v9 / v11.
    ...
    if not has_pos_emb or has_rope:
        ...
        raise ValueError(
            ...
            f"The engine binary format encodes positions as a learned embedding; "
            f"RoPE-based trunks (Veritate800M and friends) need a future format "
            f"that hasn't shipped yet. Run through the PyTorch backend instead."
        )
    if has_mtp and is_moe:
        raise ValueError(
            f"export not supported: model '{name}' has both MTP and MoE (n_experts={n_experts}). "
...
def _export_checkpoint_mtp(name, step, ckpt_path, shape, sd, base_prefix, quant_mode):
    ...
    # MTP head-0: norms[0] (RMSNorm scale-64), transforms[0] (Linear [H, H]).
    mtp_norm0     = quantize_layernorm(fetch(sd, base_prefix + "mtp.norms.0.weight"))
    mtp_trans0_w  = fetch(sd, base_prefix + "mtp.transforms.0.weight")
    mtp_trans0, mtp_trans0_q24 = quantize_matmul(mtp_trans0_w)
    ...
    with open(out_path, "wb") as f:
        f.write(struct.pack(
            HEADER_FMT,
            VERITATE_MODEL_MAGIC,
            VERITATE_MODEL_VERSION_MTP,
            shape["vocab"], shape["hidden"], shape["layers"],
            shape["ffn"], shape["heads"], shape["seq"],
        ))
        f.write(struct.pack("<i", act_boost))
        f.write(struct.pack("<iii", quant_mode, 1, 1))  # MTP path is non-MoE today
        f.write(embed.tobytes())
        f.write(pos_embed.tobytes())
        for layer in range(shape["layers"]):
            write_block(f, sd, layer, key_prefix=block_prefix)
        f.write(n_out.tobytes())
        f.write(mtp_norm0.tobytes())
        f.write(mtp_trans0.tobytes()); f.write(struct.pack("<i", mtp_trans0_q24))
        f.write(lm_head.tobytes());    f.write(struct.pack("<i", lm_head_q24))
    ...
    return {
        ...
        "shape":        shape,
        "act_boost":    act_boost,
        "version":      VERITATE_MODEL_VERSION_MTP,
        "n_experts":    1,
        "router_topk":  1,
        "quant_mode":   quant_mode,
        "mtp":          True,
