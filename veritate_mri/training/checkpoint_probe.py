...
# veritate_mri/training/checkpoint_probe.py
# ------------------------------------------------------------------------------------
# Imports:
...
def _resolve_corpus(corpus_path):
    """Return the caller-supplied corpus path if it exists. Memory probe is
    non-negotiable, so raise generically if absent rather than dropping bytes
    into a frame file with no provenance. save.save() resolves the corpus from
    the model's `corpus_stem` and passes it down, so this raises only on a
    misconfigured model dir."""
...
def dump_generation(model, prompt: str, out_dir: str, step: int,
    ...
    try:
        ...
        for _ in range(max_new):
            ...
            # Confidence + components: shared calculator in veritate_mri/confidence.py.
            ...
            conf_fields = confidence_mod.frame_fields(
                ...
                vocab=int(vocab), weights=(w_M, w_E, w_L, w_S, w_b, cw_loaded),
            )
            ...
            dla_cand   = [{"b": int(c["b"]), "entries": _dla_for(int(c["b"]))} for c in candidates]
            ...
            frames.append({
                ...
                "dla_cand":   dla_cand,
                "ablation":   None,
                "margin":           conf_fields["margin"],
                "entropy":          conf_fields["entropy"],
                "lens_consistency": conf_fields["lens_consistency"],
                "residual_stab":    conf_fields["residual_stab"],
