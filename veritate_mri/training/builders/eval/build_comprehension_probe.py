...
#   data/eval/grade/sources/grade_<level>_source.txt files.
# - 4-way MCQ over mid-passage slots; distractors are register-matched
#   (same passage), byte-length-tolerant, and excluded from the recent
#   prefix window so the model has to use long-range context.
# - Chance = 25%; bigram-only ~30-35%; real context use shows up as 50%+.
# - Output: data/eval/grade/comprehension_<level>.json with prefix_bytes,
#   correct, distractors, slot_offset per item.
# - Usage:
#     python veritate_mri/training/builders/eval/build_comprehension_probe.py
# veritate_mri/training/builders/eval/build_comprehension_probe.py
# ------------------------------------------------------------------------------------
# Imports:
