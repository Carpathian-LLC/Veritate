...
# veritate_mri/sync/models_sync.py
# ------------------------------------------------------------------------------------
# Imports:
...
# guarded by _LOCK. True while a sync() is mid-flight. Lets progress() poll
# without the multi-GB download blocking it.
...
def _stream_to_disk(branch, rel, dest, expected_size):
    ...
    bytes flow. Returns the git-blob-sha1 of what we wrote, matching the format
    GitHub's tree API returns so classify_one can compare them directly."""
    ...
    try:
        ...
        with _LOCK:
            ...
            _PROGRESS[rel]["finished_at"] = time.time()
        raise
    return sc.git_blob_sha1_file(dest)
...
def status():
    ...
    return {
        ...
        "local_files":        sc.count_local_files(MODELS_DIR),
...
def files():
    ...
    state      = sc.load_state(MODELS_DIR)
    rows       = sc.classify_set(MODELS_DIR, remote_sha, state, sha_fn=sc.git_blob_sha1_file)
    ...
    counts = sc.summarize_states(rows)
...
def sync(actions=None, branch=None):
    ...
    with _LOCK:
        if _SYNC_IN_FLIGHT:
            return {"ok": False, "error": "sync already in progress", "status": status()}
    ...
    try:
        ...
        for r in rows:
            ...
            if act == sc.ACTION_ADOPT:
                lsha = r["local_sha"] or sc.git_blob_sha1_file(os.path.join(MODELS_DIR, rel))
                if lsha is None:
            ...
            if act in (sc.ACTION_INSTALL, sc.ACTION_UPDATE, sc.ACTION_FORCE):
                ...
                if act != sc.ACTION_FORCE and st in (sc.STATE_MODIFIED, sc.STATE_CONFLICT):
                    ...
                    continue
                dest = sc.safe_dest(rel, MODELS_DIR)
                if dest is None:
                ...
                try:
                    ...
                    continue
                state[rel] = {
                    "synced_sha":    content_sha,        # git blob sha1 of what we wrote
                    "synced_at":     time.time(),
                    "remote_branch": branch,
                    "remote_sha":    remote_sha.get(rel),  # git blob sha1 from the tree
                    "size":          size,
                    "via":           act,
                }
