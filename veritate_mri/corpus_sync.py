...
# ------------------------------------------------------------------------------------
# Notes:
# - apt-style corpus library. unlike plugins/ and models/ (which are full git
#   repos), corpora are large opaque binaries downloaded one at a time over
#   HTTP into plugins/corpus/<stem>_train.bin and <stem>_val.bin.
...
def install(entry):
    ...
    Downloads files into plugins/corpus/. Verifies sha256 if provided. The
    ...
    try:
        ...
        if sha_train:
            got = _sha256_file(_train_path(stem))
            if got != sha_train:
        ...
        if val_url:
            ...
            if sha_val:
                got = _sha256_file(_val_path(stem))
                if got != sha_val:
