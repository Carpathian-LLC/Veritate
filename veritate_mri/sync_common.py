...
def classify_one(local_path, remote_sha, state_entry):
    """Return (state, local_sha) for a single file.

    - local_path:   absolute path on disk
    - remote_sha:   sha256 of remote content (None if remote unknown / not in remote)
...
def classify_set(root_dir, remote_files, state):
    ...
    for rel, rsha in remote_files.items():
        seen.add(rel)
        st, lsha = classify_one(os.path.join(root_dir, rel), rsha, state.get(rel))
        out.append({
            "path":       rel,
            "state":      st,
            "local_sha":  lsha,
    ...
    for rel, entry in state.items():
        ...
        st, lsha = classify_one(os.path.join(root_dir, rel), None, entry)
        out.append({
            "path":       rel,
            "state":      st,
            "local_sha":  lsha,
