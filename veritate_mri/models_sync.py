...
# ------------------------------------------------------------------------------------
# Notes:
# - downloads barebones model files from the public Veritate-Models repo over
#   plain https. models/ is NOT a git repo locally; this is a one-way file pull,
#   not a git operation.
# - additive only: any file that already exists locally is left alone. only
#   files that are missing locally get written. user checkpoints, hooks, and
#   trained models are never touched.
# - if models/ does not exist, it is created. if it exists with user content,
#   that content is preserved.
# - check() downloads the tarball into memory and reports counts (remote total,
#   missing locally) without writing to disk. sync() actually extracts.
