...
# ------------------------------------------------------------------------------------
# Notes:
# - MTPDecoder: wrapper around any Veritate model that exposes the cross-model
#   contract (embed, ensure_context, run_blocks, n_out) plus an .mtp submodule
#   emitting [B, T, N, vocab] logits over the next N bytes. NEVER modifies the
#   model. Three decoding strategies share the same forward path.
# - The wrapper does not maintain a KV cache. Each forward pass runs over the
#   full (rolling) context window.
# - Wired into Brain.stream() via the ?fast=mtp request mode in app.py.
# veritate_mri/inference/decode/mtp_decode.py
# ------------------------------------------------------------------------------------
# Imports:
...
class MTPDecoder:
    """Multi-byte decode wrapper. The model must implement the cross-model
    contract (embed, ensure_context, run_blocks) plus expose .n_out and .mtp
    submodules and .seq / .n_predict attributes."""
