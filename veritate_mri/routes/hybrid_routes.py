...
# ------------------------------------------------------------------------------------
# Notes:
# - OpenAI-compatible serving (/v1/chat/completions, /v1/chat/mri) plus the shared
#   ChatML prompt framing and model routing (local trained model on either engine,
#   teacher provider, or the public Carpathian model via ai_assist).
# - Retrieval is local BM25 over the knowledge base dirs (platform KB ships in the
#   repo; KB_DIR holds user files). No external embedder: the BM25 index is the
#   platform's own pure-Python retriever. Consumed by hallucination_routes.
...
_STATE = {}   # scope -> {"chunks": [...], "bm25": index}; built lazily, lives for the process
...
def _corpus_files(scope="all"):
    ...
    shipped Veritate KB, `user` is files placed in KB_DIR, `all` is both. The
    platform KB ships in the repo, so retrieval works on a fresh install."""
...
def register(app):
    ...
    def openai_chat_completions():
        """OpenAI-compatible chat completions over the shared model routing
        (_resolve_route). On a local model, temperature/max_tokens/top_k
