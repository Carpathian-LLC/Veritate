...
## addendum 2026-05-15: trainer folder + bundle entry renamed

The `plugins/` folder is now `trainers/`, and each bundle's `plugin.py` is now `trainer.py`. Manifests, `corpus/` subfolders, and CLI surface are unchanged.

- Before: `plugins/veritate_85m/plugin.py`
- After:  `trainers/veritate_85m/trainer.py`

The sync source repo is now `Carpathian-LLC/Veritate-Trainers`. Rename your local folder/files once; future syncs will pull from the new repo.

## addendum 2026-05-15: Teacher Model + Synthetic Corpus

New Settings panel **Teacher Model** plugs an external LLM into Veritate for synthetic-corpus generation. Providers: OpenAI, Anthropic, Gemini, xAI, DeepSeek, Mistral, Groq, OpenRouter, plus local Ollama / LM Studio / llama.cpp. Distill and the new Synthetic Corpus flow in the Training tab are disabled until a teacher is configured. 250 seed prompts bundle in `veritate_mri/data/seeds/` (GSM8K, MATH, HumanEval, BFCL, ARC, TriviaQA, OpenBookQA). Plugins reach the teacher via `veritate_core.plugin.get_teacher_client()`.
