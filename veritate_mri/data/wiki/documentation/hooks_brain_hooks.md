...
| mamba-2 | partial — no FFN, no attention; SSD-specific fields not yet emitted | not integrated; SSD scalar + AVX-512 kernels exist, no `model_t` yet |

### transformer field sources

| group | pytorch | engine |
|---|---|---|
| `residual_pre`, `residual_post` | block pre/post hook | `trace->residual_pre[L][pos]`, `trace->residual_post[L][pos]` (int16) |
...
Kernel selection affects perf only. Trace field shape is kernel-agnostic.
...
Five capabilities sit above the per-token frame. They are not new hooks: they are derivations and one runtime knob (ablation). All consume existing fields.
...
| live training stream | 4 | trainer flag `--mri-stream` routes per-step TFRM-lite through `save.py` | residual norms, top-K neurons, lens top-3 | none (TFRM-lite is a subset, not a new field) |

Tiers 1, 3, and 4 add no TFRM fields. Tier 2 adds two end-of-frame fields under the v7 → v8 chain bump. Tier 3's per-frame `ablation` echo is metadata, not a derivation.
