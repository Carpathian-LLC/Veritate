...
| method and path | module | params | response |
|---|---|---|---|
| `GET /generate` | backends_routes | `prompt`, `temperature` (0.7), `top_k` (40, clamped 1..256), `max_new` (200, capped 4096), `backend` (`c` default, or `pytorch`), `ablate_layer`, `ablate_neuron`, `addons` (csv), `fast` (`kv`, `mtp`, `mtp-verify`, `adaptive`), `constrained`, `adaptive_threshold`, `rep_window`, `rep_penalty`, `no_repeat_ngram`, `rag` (corpus path, loopback only), `rag_k` (max 16), `rag_compress` | SSE, described below |
