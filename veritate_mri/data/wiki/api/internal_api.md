...
| `POST /prefetch` | backends_routes | `prompt` (the exact wire prompt submit will send; omit to stand down), plus the decode knobs submit will use: `temperature`, `top_k`, `ablate_layer`, `ablate_neuron`, `addons`, `rep_window`, `rep_penalty`, `no_repeat_ngram` | `{speculating, buffered, budget, draft_id, state, stats}`, plus `reason` when the caller is not allowed or the C engine is unloaded |
| `GET /prefetch` | backends_routes | none | `{speculating, buffered, budget, draft_id, state, text, stats}` for the live draft; reading never consumes it. `state` is `running`, `done`, `budget`, `busy`, or `error` |
| `POST /prefill` | backends_routes | `messages` (OpenAI-shaped, ending on the user turn being typed; the box renders the chat prefix and fits the history itself, so send `max_tokens` when submit will) or `prompt` (raw text so far, a strict PREFIX of the wire prompt submit will send, without the closing chat scaffold), `temperature`, `top_k`; omit both to stand down | `{reading, read_bytes, stats}`, plus `reason` when the caller is not allowed or the C engine is unloaded; `400` on a malformed `messages` array |
...
| `GET /backends` | backends_routes | none | `{pytorch:{loaded, pending, model, step, last_error}, c:{loaded, pending, exe, model_bin, model_dir, blocked_reason, blocked_model, build, bins_available, warm, speculative, read_ahead}}` |
...
When a `/prefetch` draft matches the request exactly, the stream carries a `kind:"prefetch"` frame (`bytes`) after `meta`, then the buffered frames themselves. A speculative turn runs traced, so those frames carry the same telemetry a live byte does and the dashboard replays them as ordinary bytes; the engine then continues from prompt plus reply for whatever `max_new` is left.

`/prefill` adds no frames. Its effect is that the `meta` frame is followed by the first token sooner, because the prompt was already read.

**Who may work ahead.** `_ahead_allowed` splits the caller: one presenting a bearer token is programmatic (the dashboard never sends one) and is governed by `api_read_ahead_enabled` / `api_generate_ahead_enabled`; this box's own UI is governed by `read_ahead_enabled` / `speculative_enabled`. The split exists because the box runs a single stateful engine, so a client working ahead is a client holding it.
...
On a local model, `/v1/chat/completions` and `/v1/chat/mri` fit the conversation to the model's context before generating: oldest whole turns drop until the rendered prompt fits, with the turn being answered always kept. `max_tokens` sizes the reply room reserved from that context, so raising it keeps fewer turns. Without the fit, the engine would cut the prompt at a byte offset and the model would answer whichever earlier turn survived.
