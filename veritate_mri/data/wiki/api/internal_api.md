...
| `POST /prefill` | backends_routes | `prompt` (the text so far, a strict PREFIX of the wire prompt submit will send, without the closing chat scaffold; omit to stand down), `temperature`, `top_k` | `{reading, read_bytes, stats}`, plus `reason` when the caller is not allowed or the C engine is unloaded |
