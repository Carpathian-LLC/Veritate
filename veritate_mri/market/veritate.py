...
# veritate_mri/market/veritate.py
# ------------------------------------------------------------------------------------
# Imports:
...
_MRI = os.path.normpath(os.path.join(HERE, ".."))
...
try:
    from tools import series_codec as sc
except ModuleNotFoundError:                       # standalone (tests) — tools dir on path
    sys.path.insert(0, os.path.join(_MRI, "tools"))
