...
// V_SEQ is overridable from the build (-DV_SEQ=1024 for hybrid-class models).
#define V_VOCAB     256
#ifndef V_SEQ
#define V_SEQ       256
#endif
...
typedef struct {
    ...
    float    cw_b;
    int32_t  cw_loaded;
    // v13 hybrid trunk. NULL for v3-v12. when set, forward / forward_decode /
    // sample_token_ext route through src/hybrid.c and the int8 fields above
    // (embed, blocks, scratch, lm_head) stay NULL.
    void*    hybrid;
} model_t;
...
// repetition control (chat decode). byte-level no-repeat-ngram hard ban plus a
// soft suffix-repeat penalty over a sliding window of generated bytes. mirrors
// veritate_mri/inference/decode/repetition.py; keep constants + algorithm in sync.
#define VERITATE_REP_MIN_MATCH  4
#define VERITATE_REP_MATCH_CAP  64

typedef struct {
    const uint8_t* hist;             // generated bytes so far (this turn)
    int32_t        hist_len;
    int32_t        window;           // sliding window; <=0 uses full hist
    float          penalty;          // soft demotion, nats per byte over min_match
    int32_t        no_repeat_ngram;  // hard-ban n-gram size; 0 disables
} rep_ctx_t;

// tied-embedding lm head + sampler. temp<=0 argmax. top_k<=0 no filter. rep NULL
// disables repetition control (the sampler is then bitwise-identical to before).
