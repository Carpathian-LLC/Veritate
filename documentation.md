...
Branch resolution (`_tracked_branch`): in a git checkout the checked-out branch wins over the channel branch, but only while the remote still has it. `_remote_branch_sha` asks the commits endpoint and returns `(sha, missing)`; `missing` is True only for a 404 or 422, so an offline box or a rate limit never retargets a box onto another branch, and `/branches/<name>` is unusable here because GitHub 301s an unknown branch onto the default one. A checkout left on a branch deleted upstream therefore follows its channel: GitHub serves the archive URL of an unresolvable ref with the DEFAULT branch's tarball, so tracking such a branch installs `main` under the old name while every compare against the missing ref 404s, which reads as up to date and leaves the update button disabled (measured on cardinal-01, stranded on `master` 40 commits behind `dev`; pinned by `tests/mri/test_updater_branch.py`). `status()` never resolves the branch itself - it reports `remote_branch` as the last check recorded it, because the dashboard polls status once a minute per open tab.
...
Single-page app: `veritate_mri/web/index.html` + `index.js`, plus one module per concern that has outgrown a screen of code: `image_mri.js` (the image-model MRI, `window.ImageMri`), `generation_images.js` (the Generation tab's image layout), `image_live.js` (the Training tab's live view of an image run, `window.ImageLive`), `tutorial.js`, `prune.js`, `rag_train.js`, `wiki.js` (the Wiki tab, `window.Wiki`), each with its own `.css`. A module is an IIFE behind one `window.<Name>` export, reads no `index.js` symbol, and is loaded by a plain `<script>` tag after `index.js`; `index.js` reaches a module only through its export and only at run time, so load order never matters. New frontend work of any size goes in a module, never in more `index.js` lines; `tests/mri/test_web_modules.py` pins the pattern. Tabs: Generation, Models, Training, Distillation, Wiki, Logs, Settings. The chat tab was extracted 2026-08-20 to a separate project (see `CHAT_HANDOFF.md`); the Generation tab is the conversational surface. Hash-based navigation; the `valid` array in `index.js` is the sole allowlist of routable tabs — an unknown tab silently falls back to `generation`. Adding a tab: add to `valid`, add an `activateTab` branch, and update the tutorial selectors (renaming a tab breaks the tutorial spotlight silently). `MINIMAL_HIDE_TABS` hides tabs in minimal mode.
...
- Standalone modules (image_mri, generation_images, prune, tutorial, rag_train, wiki) use the IIFE pattern.
- Generation tab, what to generate: the `text | images` switch at the top of the tab (`name="genKind"`, persisted per browser as `vt:generation:kind`) swaps the whole tab. Text is the conversational surface below with its byte-level MRI. Images puts `is-images` on the tab body, which hides every other child and shows `#imgGenView`: the draw composer, the MRI of the picture just drawn, and the checkpoint MRI of the model drawing (`ImageMri.create("imgGenMri")`, the same view the Models tab shows). Owned by `generation_images.js`; the text layout and its tiers (chat / agent / autocomplete) are untouched by it.
...
- Wiki tab: `wiki.js` renders this file. `GET /wiki` is the section list (`##` and `###` headings, first slug wins) and `GET /wiki/doc` the whole rendered document; the tab fetches both once per load. Layout is a sticky contents rail plus one article that scrolls with the page, never a nested scroll box. The rail nests `###` entries under their `##` group, filters on typed text (a group stays visible while any child matches), and tracks the heading nearest the top of the viewport. Nav entries jump by heading `id`, which `readers/wiki.py` slugs from the heading text, so every `##`/`###` heading in this file must be unique or the duplicate is unreachable from the rail. Prose renders proportional at a 78ch measure; code, tables and paths stay monospace. `/wiki/<slug>/page` serves one section as a standalone page for the Training form's `learn more` links, styled to match the dashboard, with the section's own heading promoted to the page title.
...
`veritate_core/plugin/image_sample.py`; `POST /images/generate`; the Generation tab with **images**
picked as what to generate. One mechanism serves every mode, because the model was trained to fill masked
...
**The MRI of one picture.** `trace: true` in the request body makes `fill()` record every pass and the answer carries `trace`: `passes` (one entry per pass: `pass`, `committed` cells, their mean `confidence`, and a `png` of the picture at that pass with the still-undecided cells painted `MASK_GREY`), `commit_pass_map` (the pass each plane-0 cell was decided in), `confidence_map` (each cell's probability at the pass it was committed, averaged over its planes; kept cells from a source photo read as certain), `grid` (`[gh, gw]`) and `codes_used`. `image_sample.trace_report` builds it; `trace_frames`, `formation_order` and `grey_cells` are the same functions the checkpoint probe draws `passes.png` and `formation.png` with. The Generation tab shows the passes as a strip, the commit-pass and confidence maps as fixed-cell heat maps, and cells and confidence per pass as bars, under the picture, the way the byte panels sit under a reply. A generation without `trace` pays nothing for it.
...
**The view is `veritate_mri/web/image_mri.js`** (`ImageMri.create(hostId, {onContinue})` -> `show(name)` / `hide()`), one instance per host: the Models tab creates one for `#imgMri`, the Generation tab's image layout one for `#imgGenMri`, and the Training tab's live view (`image_live.js`) draws with the same figure helpers (`ImageMri.charts`: `svgLine`, `barChart`, `cellHeat`, `scaleBar`, `photoTile`, `gridRows`, `wireLightbox`; `ImageMri.colors`). Styles are `image_mri.css`.
...
Off by default. On, every `QuantLinear` fake-quantizes its weight in the forward (`qat.set_qat`)
so the trained weights already sit on the rounding grid the export writes; the model is named
`<slug>_qat` and `config.json` records `training: qat` with the source model of a QAT continue.
Resuming a plain model under QAT loads `strict=False` because the quantized layers own observer
tensors the source lacks.

### quant_mode

`int8` (default), `int4` or `ternary` (BitNet 1.58): the weight rounding QAT trains against
(`qat.QUANT_MODES`, applied to every quantized layer by `qat.set_quant_mode` right after
`set_qat`). Read only when `qat_enabled` is on; an unknown value refuses the launch. The export
scheme must match: a ternary-trained model exports ternary, an int4-trained one int4, and int8
QAT targets the v9 per-tensor scheme, not v13's per-row scales (see the engine section).
...
Off by default. On, every training window drawn from the corpus mix slides forward to the next `<|im_start|>user` inside it, so a chat window opens on a turn boundary; a window with no marker in reach, and any corpus without markers, draws as before. Without it a window can open on a question whose answer was told before the window began, and the loss then trains a guess: on the recall corpora that guess surfaced as invented facts in the committed-state and leak conditions of the working-memory probe (lab 2026-09-05-working-memory-program). Pair it with `loss_mask assistant` on any conversation corpus whose answers depend on earlier turns. `align_stride N` (int, 0 off, wins over `align_windows`) opens every window on a multiple of N bytes instead; with a corpus whose records are padded to N (`build_fact_chats --pad-to N`) each window is one whole conversation, so a question is never drawn without the telling that answers it. Prose corpora in the same mix are unaffected by either setting beyond where their windows start. The run header states the result as `window align: stride N B` / `user turn` / `none`, because a finished run's log is the evidence of where its windows opened.
