...
**int8 is 1.46-1.72x on a bandwidth-limited box, but it CHANGES OUTPUTS (see failures.md)**
Cardinal, 128 B greedy: wren1_0 fp16 **15.17 ms/byte** vs wren1_0_int8 **10.42 ms/byte** (1.46x); the win lands on the boundary step (41.34 -> 30.13 ms), which is where the bandwidth goes. End to end on wren1_3, int8 plus 8 pinned engine workers: **17.5 -> 10.2 ms/byte, 1.72x**. Greedy parity is FALSIFIED (failures.md 2026-08-23): fp16 and int8 exported from one checkpoint agree on 1 of 5 replies. int8 is the first lever on any bandwidth-limited box but it is not free -- it needs a quality eval, not a parity check. Separately, engine-direct instrumentation puts the entire Python pipeline (frame read, parse, event dict, JSON, SSE, socket) at **0.02 ms/byte, 2 ms of 2,198 (0.1%)**: the transport is not the bottleneck, and coalescing SSE frames would buy nothing. Confirmed by an A/B against the non-streaming `/v1` path, which measures the same 18.6-21.5 ms/byte. (2026-08-23)
...
**Per-class engine thread calibration shipped; best-so-far rung selection stabilized the pick, the per-class switch is inert on cardinal (IDEA 22, 2026-08-24)**
`hybrid.c` times both step classes, compares each rung against the best so far, and exposes `VERITATE_HYBRID_CALIB_KNEE` / `VERITATE_HYBRID_BOUNDARY_THREADS`. cardinal at 2.0 GHz, 1/2/4/8 workers: plain 10.742/6.261/4.775/**3.939** ms/byte, boundary 57.662/34.511/27.320/**23.144** -- a boundary step costs 5.9x a plain one, but both classes pick 8, so the switch gains 0% here (falsifier >5% not met; retry on a box where the classes diverge). Parity holds through the mid-step switch. Full record in ideas.md history (git) and `documentation.md` engine section.
...
**The full 6,000-step chat SFT brought it to parity, and the wsd decay tail is where the fluency came from.** Mid-run (rung 146,000) it was closure 0.938, loop 0.438, grounded 0.312, identity 0.812 against the 270M's 1.000 / 0.250 / 0.250 / 0.750 - composite 0.656 vs 0.688, losing. The last 1,500 steps of sqrt decay from 2e-5 to 2e-6 changed the picture: **loop 0.438 -> 0.188** (the 598M now loops LESS than the 270M), closure 0.938 -> 0.958-0.979, identity held 0.812. Best rung **wren2_0@149,500, composite 0.693 vs the 270M's 0.688**. The tail was not free: **grounded retrieval fell 0.312-0.375 -> 0.188**, below the 270M's 0.250, monotonically across all four tail rungs - annealing bought turn discipline by trading context-following. At 0.5% the composite margin is inside the rule-30 noise floor, so the defensible claim is parity overall, a clear persona and anti-looping win, and a real retrieval regression. Val was flat across the whole anneal (0.35501 at 148,500 -> 0.35674 at 150,000), so **val could not have selected this rung** - only the behavioural ladder saw it. Lab: 2026-09-13-wren2-chat-sft-and-scale-benchmark.

**A decay tail can improve every fluency metric while silently degrading retrieval (2026-09-17)**
Same run, four rungs 148,500/149,000/149,500/150,000. Loop 0.292/0.354/0.188/0.229 and closure 0.958/0.979/0.958/0.979 both at run-best, while grounded sat at 0.188/0.250/0.188/0.188 against 0.312-0.375 before the anneal. A composite score hides this - it moved 0.656 -> 0.693 while one of its four terms halved. Rule: score a decay tail per axis, never on an aggregate, and keep a pre-anneal rung when retrieval matters more than polish.
...
Rules: (1) the val transient is real — one reading above the line is not damage, the stop rule needs two consecutive readings (shipped); (2) a freeze count is model-shape-specific, so it is a per-install sleep setting, never a global default; (3) judge forgetting on a full pass over the val file, not on a draw the size of the file.

**Knowledge consolidation improves general capability instead of costing it; fusion measured inert (wren1_12, 2026-08-26)**
150 steps of code QA at lr 5e-6 vs the pre-run weights (`tools/val_eval.py`, seeded draw, exact windows): mixed_chat −2.37…−3.85%, veritate_chat −2.86…−6.00% — better at every checkpoint on both corpora. The 3.36% "drift" that tripped `stop_on_val_rise` was a rise above the run's own best, not below its start. Fusing step 150 toward step 0 (`training/fuse.py`, α 0.25/0.50/0.75) improved capability AND retention monotonically with α — no knee, nothing to repair, full α best. Rule: measure a consolidation run against its starting weights; reserve fusion for runs that actually degrade. `val_eval` reproduced a `train.csv` reading to six decimals (0.946437 on veritate_chat), validating the instrument.
