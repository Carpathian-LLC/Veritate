...
Five hot-path primitives split out of `veritate_engine/src/model.c` into
`veritate_engine/kernels/<arch>/`:
...
| `score_dot_v_avx512`    | direct call     | `veritate_engine/kernels/x86_64/transformer_avx512.c` |
| `softmax_rows_avx512`   | direct call     | `veritate_engine/kernels/x86_64/transformer_avx512.c` |
| `layernorm_i16_to_i8_avx512` | direct call | `veritate_engine/kernels/x86_64/transformer_avx512.c` |
...
**Step 1, `veritate_engine/src/veritate.h`:** add the typedef and extern next
to the matmul block.
...
**Step 2, `veritate_engine/src/dispatch.c`:** add the global default and the
runtime selection.
...
**Step 3, `veritate_engine/src/model.c`:** swap call sites from the direct
...
`grep -n score_dot_v_avx512 veritate_engine/src/model.c` finds them; same drill
for `softmax_rows_avx512` and `layernorm_i16_to_i8_avx512`.

That's the whole wiring. About 30 seconds of editing. The AVX-512
implementations stay where they are, in `veritate_engine/kernels/x86_64/transformer_avx512.c`,
...
2. **Extract from `model.c` to `veritate_engine/kernels/x86_64/`**: keep current
   AVX-512 work, refactor as standalone `.c` files exposing the
   function-pointer signatures. Add `veritate_engine/kernels/scalar/` references for
...
5. **AVX2 port**: implement the same five functions in
   `veritate_engine/kernels/x86_64/matmul_avx2.c` style. Old Mac mini Intel + old PC.
6. **NEON SDOT port**: `veritate_engine/kernels/arm64/matmul_neon_sdot.c`. Apple Silicon mini, M4
   Studio, modern Android.
7. **NEON-only port**: `veritate_engine/kernels/arm64/transformer_neon.c`. Pi 4 baseline.
