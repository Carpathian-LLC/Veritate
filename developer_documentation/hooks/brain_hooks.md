...
Which hook fields are available on which architecture and engine path. Field shapes are identical across paths so the MRI dashboard renders one shape regardless of source. Field index lives in [contract.md](contract.md).
...
|---|---|---|
| scalar | `veritate_engine/kernels/scalar/` | reference oracle for every architecture; parity bar for x86_64 / arm64 ports |
| x86_64 avx-512 vnni | `veritate_engine/kernels/x86_64/` | transformer matmul (qkv, out_proj, ffn_up, ffn_down, lm_head); mamba-2 ssd kernel prototype |
| arm64 neon | `veritate_engine/kernels/arm64/` | planned, transformer matmul first |
...
4. If the field changes the TFRM frame size, bump the trace version in `veritate_engine/src/veritate.h` and update the parser.
