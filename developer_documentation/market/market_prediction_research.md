...
## E. OFI direction x magnitude-gate composition, fee sweep to maker level (2026-07-02)

Decisive composition backtest: pooled walk-forward models (logreg + HistGB direction, HistGB magnitude) over 40 Binance USDT majors, 1m bars resampled to 15m/1h, test years 2022-2026, fees swept 0/1/2/3.5/5/10 bp per side with position-change (not per-bar) fee accounting. Smoke: `SMOKE_RESULTS/ofi_magnitude_composition_smoke.py`, stats: `SMOKE_RESULTS/ofi_magnitude_composition_stats.json`.

- **Composition confirmed on gross:** magnitude gating multiplies gross bp/trade 3-7x (1h gbm ungated 0.8-1.7 bp -> mag+conf gate 2.2-12.4 bp; 15m up to 6.6 bp). Magnitude model real every year (Spearman pred-vs-realized 0.35-0.43). Shuffled-label nulls clean (AUC 0.496-0.506).
- **But gating raises turnover per trade** (0.4-0.7 -> 1.3-1.6 sides/trade: gated bars are scattered, nearly every trade pays a full round trip), so the fee floor moves less than gross does.
- **Falsifier at 3.5 bp/side: HIT.** No cell (either horizon, any gate, either model, per-pair or cross-sectional) is net-positive at 3.5 bp/side in >=3 test years with both halves positive. Nothing survives at 2 bp/side either.
- **The edge survives at 1 bp/side:** best cell 15m gbm mag80+conf80 gate: net +1.1/+5.0/+3.2/+0.3/+2.1 bp per trade 2022-2026, both-halves-positive in 4/5 years (2025 h2 fails), 74-170 trades/day across 40 pairs, roughly 2-34%/yr unlevered on 1/40-split capital. 1h gbm mag_q80 also survives at 1 bp (2023-2025) but is dead in 2026 (1h AUC decayed 0.533 -> 0.511; the 15m signal has not decayed).
- **Cross-sectional 1h arm (top8/bottom8 by score): dead net of fees.** Gross 0.2-2.2 bp/bar but turnover ~1.1-1.3/bar; net-negative at >=1 bp/side in most years, always at 3.5.
- **Verdict:** the fee-floor attack on the direction edge fails at taker/retail fees (>=2 bp/side); composition rescues it only at <=1 bp/side, i.e. futures VIP-maker/rebate execution with passive fills, which adds non-fill and adverse-selection risk the backtest does not model. Retry condition: an execution path with all-in cost <=1 bp/side, or a magnitude gate that clusters trades in time (turnover per trade back under ~1).
- **Dataset landmine (affects any consumer of `extensions/installed/market/data/crypto_of/`):** the `time` column switches from milliseconds to microseconds at 2025-01-01 (Binance vision format change). Raw reads misparse all 2025+ bars; normalize with `time = time // 1000 where time > 1e14`. Gap check post-fix: ~2.3k missing minutes per pair over 2020-2026, max gap 355 min, exchange-wide outages; features/labels must not span gaps.

---

## F. Options monetization of the magnitude signal: straddle-proxy scoping (2026-07-02)

Question: can US retail monetize the (real) realized-vol/magnitude signal through listed crypto options? Instrument scan (web, 2026-07) + daily long/short ATM straddle proxy on BTC/ETH 1m data, walk-forward expansion signal, sweeping VRP multiplier x spread cost x gate threshold. Smoke: `SMOKE_RESULTS/vol_options_scoping_smoke.py`, stats: `SMOKE_RESULTS/vol_options_scoping_stats.json`.

- **Instruments (US retail, 2026):** IBIT (Nasdaq) is the deepest BTC options venue globally (passed Deribit OI late 2025), penny-increment quotes, Mon/Wed/Fri expiries. ETHA weeklies (wider). CME micro BTC options exist (0.1 BTC) but are thinner than IBIT. Deribit is geo-blocked for US persons; Coinbase FCM opened a regulated route 2026-05-29, institutional-only so far. BITX is 2x-leveraged BTC futures ETF, not a vol product; no listed BTC-vol ETP exists.
- **Real spreads (CBOE delayed chain, 2026-07-02, IBIT $35):** nearest-expiry ATM straddle mid $0.435, combined spread $0.05 = ~11% of premium; 2-trading-day straddle mid $1.06, spread $0.06 = ~5.7% round trip, ~2.8% hold-to-expiry (entry half-spread only). The "50-100 bp of premium" cost floor scoped in the mission does not exist for near-dated crypto straddles; realistic is 300-600 bp of premium.
- **Published VRP:** BTC implied variance exceeds realized on average: BVRP ~14 annualized variance points (risk-neutral 0.72 vs realized 0.58, IV/RV vol-ratio ~1.11, Deribit 2017-2022, arXiv 2410.15195); Bitcoin-VIX study reports ~+15 vol pts of overpricing in contango regimes (ratio 1.2-1.3). Snapshot now: IBIT IV30 40.1% vs BTC RV30 26.6% = ratio ~1.5.
- **Long straddles gated on the signal (thr 0.8, ~65 trades/yr): DEAD.** With IV anchored to trailing 30d RV, positive only at VRP<=1.1 (+4 bp/trade at 300 bp cost, sign flips by year); at published VRP 1.2/1.3: -17/-38 bp per trade (BTC), -24/-52 (ETH). With IV anchored to trailing 5d RV (reactive pricing: vol clustering is public, market makers see yesterday's expansion too), negative EVERYWHERE, even at VRP=1.0 and 50 bp cost (-16 to -21 bp). Buying every day bleeds -48 bp/trade at VRP 1.1, as VRP theory predicts.
- **Short straddles gated on predicted-quiet days: the apparent edge is the VRP, not the signal.** Under rv30-anchored IV the gated short shows +60-96 bp/trade every year, but under rv5-anchored (reactive) IV it collapses to ~0 (-4.5 BTC / -1.4 ETH bp at VRP 1.1) and is WORSE than selling unconditionally (+14/+16 bp). Tail: worst single day -800 to -1700 bp of underlying notional; one tail eats 15-40 average wins, plus IBIT weekend gap risk (crypto trades through weekends, options do not) and naked-short margin tiers.
- **Signal-horizon mismatch (new finding):** the magnitude edge decays sharply with horizon: Spearman 0.35-0.43 at 15m/1h (section E) but only 0.04-0.19 at daily (expansion signal 0.02-0.19, rv5-level 0.04-0.16, per year 2022-2026, both symbols). The shortest expiry US retail can trade (1-2 day IBIT) sits exactly where the signal is weakest, and 0DTE spreads (~11% of premium) foreclose intraday use.
- **Falsifier: HIT. Options monetization of the magnitude signal is dead for US retail.** The signal predicts what IV already prices (recent vol expansion); at published VRP and real IBIT spreads there is no long-vol expectancy, and the short-vol side is plain VRP harvesting with -8% to -17% single-day tails, no lift from our signal. Retry condition: an options venue with intraday (15m/1h) expiries at <=100 bp-of-premium round trip for US retail (does not exist as of 2026-07), or evidence that real IBIT IV lags realized expansion by >=1 day (testable with an IV history feed, not free).
