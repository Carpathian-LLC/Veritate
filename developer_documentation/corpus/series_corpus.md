...
`LOADERS` table ([build_series_corpus.py](../../extensions/canonical/trading/server/build_series_corpus.py)).
...
- `indices` — daily cash index levels (^GSPC, ^NDX, ...) from the Yahoo v8 chart
  feed; `adjclose` mirrors `close`. Puller: `extensions/canonical/trading/server/pull_yahoo.py indices`.
- `futures` — daily Yahoo continuous front-month contracts (ES=F, CL=F, ...), the
  ...
  `extensions/canonical/trading/server/pull_yahoo.py futures`.
...
- `crypto_extra` — every other Binance Vision USDT pair (full archive minus the
  ~200 already in `crypto`). Puller: `extensions/canonical/trading/server/pull_binance.py --full`.
- `crypto_1s` — the same pairs at 1-second resolution (the high-frequency tape).
  Reads `extensions/installed/trading/data/market/crypto_1s/*.csv`; shares the crypto loader. The serving
  layer routes a `base=1s` request to this `_1s` tape and reads it raw with no
  resample ([data.py:42](../../extensions/canonical/trading/server/data.py#L42)).
- `forex` — 1-minute major-pair bars resampled from Dukascopy ticks (crypto
  schema; volume is tick count). Puller: `extensions/canonical/trading/server/pull_forex.py`. Writes
...
Codec and feature math: [series_codec.py](../../extensions/canonical/trading/server/series_codec.py).
Builder: [build_series_corpus.py](../../extensions/canonical/trading/server/build_series_corpus.py).
...
([data.py](../../extensions/canonical/trading/server/data.py)), shared by the builder and the serving
path so the on-disk and live formats never diverge. Funding is per-symbol
(`extensions/installed/trading/data/market/funding/<SYM>.csv`, columns `time,funding`); sentiment is global
(`extensions/installed/trading/data/market/sentiment/fng.csv`, columns `time,value`). The join is
...
[series_codec.py compute_features](../../extensions/canonical/trading/server/series_codec.py).
...
[test_series_codec.py](../../extensions/canonical/trading/tests/test_series_codec.py).
...
Raw data lives in the gitignored `extensions/installed/trading/data/market/{stocks,crypto,crypto_1s}/`. Then:
...
```
python extensions/canonical/trading/server/build_series_corpus.py --source crypto_of --horizon 15m
python extensions/canonical/trading/server/build_series_corpus.py --source crypto_of --horizon 5m
python extensions/canonical/trading/server/build_series_corpus.py --source crypto_of --horizon 1h
```
...
`extensions/installed/trading/data/market/crypto/` CSVs are OHLCV-only, so those two channels encode the
...
```
python extensions/canonical/trading/server/fetch.py BTCUSDT ETHUSDT SOLUSDT --bars 400000 --source crypto_extra
python extensions/canonical/trading/server/build_series_corpus.py --source crypto_extra
```
...
**Context-channel corpus (funding + sentiment).** `extensions/installed/trading/data/market/crypto_of/` carries
the 40 majors with order-flow columns; pulling funding
(`extensions/installed/trading/data/market/funding/<SYM>.csv`, Binance Vision monthly dumps 2020+) and the
fear-greed index (`extensions/installed/trading/data/market/sentiment/fng.csv`, alternative.me, 2018+) lets the
...
- `extensions/installed/trading/data/market/` raw CSVs (gitignored, local only).
...
- **Stamp = codec stride at save time.** Build the corpus and train in the same checkout
  ...
  build a crypto corpus after pulling `extensions/installed/trading/data/market/funding/` + `extensions/installed/trading/data/market/sentiment/`
