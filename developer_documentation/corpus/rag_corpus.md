...
- `build(n_facts, qa_per_fact, stem)` renders train examples through `TEMPLATE`, writes `<stem>_train.bin` and `<stem>_val.bin` (`VAL_FRAC` tail) into `data/corpus/`, and writes the held-out items to `veritate_mri/data/eval/rag/<stem>_test.json` (`paths.rag_eval_path`).
