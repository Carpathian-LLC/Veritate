...
Apt-style installer for training corpora, surfaced as the "Corpus library" card on the Settings tab. Resolves a catalog of corpora and installs each into `data/corpus/<stem>_train.bin` / `<stem>_val.bin`, where the Training tab corpus picker finds them.
...
`uninstall()` deletes the two `.bin` files from `data/corpus/` only. For native corpora the repo copy stays, so reinstall always works.
