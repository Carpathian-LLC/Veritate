# trading market routes: byte-model serving API

The market route group of the Trading extension (`extensions/canonical/trading/`,
see [trading_extension.md](trading_extension.md)): the `/ext/trading/market/*` section of
`extensions/canonical/trading/server/register.py`. The extension registry
(`extensions/registry.py`) auto-discovers the extension, inserts `server/` onto `sys.path`,
imports `register.py`, and calls `register(app)`. The model-analytics page consuming these
routes is served at `/ext/trading/models` (`page/models.html`). Server modules import
...
All handlers wrap their body in a local `_safe(fn)` helper in `register.py` for the
...
- **GET `/ext/trading/market/veritate_models`** -> `{models:[...]}`: every byte-level Veritate model that
...
- **GET `/ext/trading/market/veritate_hindcast?source&symbol&model&base&n`** -> price-space overlay
...
- **GET `/ext/trading/market/veritate_benchmark?source&symbol&model&base&n`** -> the full scored metric set
...
- **Resolution (`base`).** `base` selects bar size: `1m, 5m, 15m, 1h` (default `1m`). All read the
  1m-native `extensions/installed/trading/data/market/crypto/<symbol>.csv` and resample to coarser bars. The page is
...
- **On-demand backfill (no manual data).** A fresh install ships no data dir. When
  ...
  them in `data.py`'s schema, and caches to `extensions/installed/trading/data/market/crypto/<symbol>.csv` (later loads are
...
- **GET `/ext/trading/market/veritate_live?source&symbol&model`** -> next-bar forecast
...
## Paper-trading endpoints (drive the Research tab + news trader chart gate)
...
`register._policy_args`. The Trading page's Research tab and `news_trader.chart_ok` are
the only callers; the Research tab consumes these over HTTP and holds its paper ledger in
the browser.

- **GET `/ext/trading/market/paper_signal?source&symbol&model&base&n`** -> the raw per-bar forecast
  (`veritate.signal_series`): `t`, `price`, `p_up`, `conf`, `exp_move`, `vol`, `ret_next`,
  `n`, `base`, plus `symbol`/`model`/`step`. No policy applied. This is what the Research
  tab fetches (and caches): it runs the policy in the browser so rule tweaks,
...
- **GET `/ext/trading/market/paper_backtest?source&symbol&model&base&n&mode&fee_bps&conf_gate&move_gate&sizing`**
...
- **GET `/ext/trading/market/paper_decide?source&symbol&model&mode&fee_bps&conf_gate&move_gate&sizing`**
...
- **GET `/ext/trading/market/veritate_data_report?source`** -> cheap per-instrument inventory
...
- **GET `/ext/trading/market/instruments?source=crypto`** -> `{ok, source, instruments:[...]}`: local raw 1m
  CSVs under `extensions/installed/trading/data/market/crypto/` unioned with `fetch.MAJORS` (`data.list_instruments`), so the
