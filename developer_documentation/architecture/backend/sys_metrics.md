...
`snapshot()` returns a lightweight snapshot for the HUD: per-component usage and temperatures. Sampled at a moderate cadence; cached briefly. It is served by `GET /sys_metrics`.
...
- [routes/sys_routes.py](../../../veritate_mri/routes/sys_routes.py): exposes `/sys_metrics` (`snapshot()`), `/sys/specs` (cached specs), `/sys/detect` (re-detect and persist), plus `/sys/mode` and `/sys/mode/relaunch` for the minimal-mode toggle.
...
- GPU temperatures are best-effort; absent fields render as a dash placeholder in the HUD rather than 0.
