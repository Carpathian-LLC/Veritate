...
9. **App-sync reload hook**: `app_sync.set_reload_hook(_app_sync_reload)`, where `_app_sync_reload` calls `lifecycle.restart(app.config)` so a remote code push triggers a restart. See [lifecycle.md](lifecycle.md).
...
- `VERITATE_MINIMAL=1` must be set in the environment BEFORE the Python process starts; runtime changes are ignored.
