...
Auto-installer for missing Python packages, torch wheel repair, and native OS helpers, at [veritate_core/plugin/deps.py](../../../veritate_core/plugin/deps.py). Backs the Auto tune modal's missing-dep recovery, the Detect Hardware modal's torch-build repair, and the HUD sensor row's auto-install of temperature helpers, so unblocking a benchmark or unlocking the temperature HUD never requires a trip to a terminal. Kept in sync with the one-shot launcher install in [veritate.py](../../../veritate.py), which handles the first-boot bootstrap.

## How it works

### Python packages

- [`is_installed(pkg)`](../../../veritate_core/plugin/deps.py#L173): trivial `importlib.import_module` check. `pkg` is treated as the import name (with `-` becoming `_`). Callers whose dist name differs from the import name (`opencv-python` vs `cv2`) must check via `importlib` directly.
- [`ensure(pkg, index_url=None)`](../../../veritate_core/plugin/deps.py#L307): idempotent wrapper. When already importable, returns `{ok: True, method: "present"}` without invoking pip.
- [`install(pkg, index_url=None)`](../../../veritate_core/plugin/deps.py#L246): escalation ladder, stops at the first success:
    1. `pip install --user <pkg>` (no elevation).
    2. `pip install <pkg>` (site-packages when the invoking account has write access).
    3. Windows: [`ShellExecuteW(runas)`](../../../veritate_core/plugin/deps.py#L207) triggers a UAC prompt. Automated in the sense that no CLI-flag confirmation is needed; the OS-level admin consent is unavoidable.
...
The single most common bad-install case: a Windows or Linux-x86 box with an NVIDIA GPU ends up running the CPU torch wheel (pip fell back to PyPI). The runtime helpers below let the Detect Hardware modal see the mismatch and repair it in-place without a reinstall from scratch.

- Constants [`TORCH_CUDA_INDEX`](../../../veritate_core/plugin/deps.py#L48) (`https://download.pytorch.org/whl/cu128`) and [`TORCH_CPU_INDEX`](../../../veritate_core/plugin/deps.py#L49) (`https://download.pytorch.org/whl/cpu`): the wheel indices used by `install_torch()`. Duplicated (not imported) from the launcher so a runtime repair still works when the launcher module isn't on `sys.path`.
- [`has_nvidia_gpu()`](../../../veritate_core/plugin/deps.py#L328): probes for a usable NVIDIA GPU **without** importing torch; this is the signal that *decides* whether to reinstall torch. Cross-checks signals so a working GPU + driver still routes to the CUDA build when `nvidia-smi` isn't on PATH:
...
- [`torch_state()`](../../../veritate_core/plugin/deps.py#L410): introspects the running interpreter's torch. Returns `{installed, version, cuda_build, cuda_runtime, cuda_available, device_count}`. Import failure counts as `installed: False`.
- [`install_torch(force_cuda=None)`](../../../veritate_core/plugin/deps.py#L466): reinstalls torch from the correct wheel index for the running box:
...
- [`status_snapshot()`](../../../veritate_core/plugin/deps.py#L543): one-shot dict for the modal so a single `POST /sys/detect` produces everything the frontend needs to decide whether to auto-open the installer popup:
...
- [`_temp_sensor_helper_id()`](../../../veritate_core/plugin/deps.py#L612): picks the temperature-sensor helper matching the running arch: `mac_temp_arm` on Apple Silicon, `mac_temp_intel` on Intel Mac, `linux_lm_sensors` on any Linux with `apt-get`, else `None`. Windows returns `None` because LibreHardwareMonitor ships as a GUI-only installer, surfaced via a link in Settings rather than this auto-install path.

### Native OS helpers

Some HUD features (CPU/GPU temperature) depend on OS-level tools pip can't install. [`HELPERS`](../../../veritate_core/plugin/deps.py#L88) is a keyed registry so a caller doesn't reason about brew/apt package names:
...
[`install_helper(helper_id)`](../../../veritate_core/plugin/deps.py#L629) dispatches to `_run_brew` or `_run_apt`. Guards against wrong-OS invocation (returns `unsupported: True`), missing package manager (brew not on PATH reports that Homebrew must be installed first), and passwordless-sudo prompts (`-n` on apt). Windows LibreHardwareMonitor is not auto-installable because it ships as a GUI-only installer; the Settings tab renders the manual instructions for that case.

### The launcher install

First-boot bootstrap lives in [veritate.py](../../../veritate.py) and runs once per venv (guarded by a requirements hash sentinel). It creates a venv, then installs deps from [requirements.txt](../../../requirements.txt) via the two-phase pattern in [`_install_torch_then_rest(py, index)`](../../../veritate.py#L627):

- **Phase 1:** `pip install --index-url <pytorch-index> torch`. This is the load-bearing choice: `--index-url` (not `--extra-index-url`) locks pip to the pytorch wheel index for this call only, so the resolver can't pick the same-versioned CPU wheel from PyPI. With `--extra-index-url`, boxes with NVIDIA GPUs land on CPU torch because PyPI stays eligible.
- **Phase 2:** `pip install -r requirements.txt` from PyPI. Torch is already satisfied, so pip won't try to re-resolve it and the CUDA wheel just installed stays put.
- **Phase 3 (CUDA path only):** [`_verify_torch_cuda(py)`](../../../veritate.py#L520) runs `import torch; torch.cuda.is_available()` in a subprocess (the launcher's own interpreter may not have torch on `sys.path` yet). False here means the wheel is CPU-only, or the driver is too old, or a stale CPU torch is shadowing the venv; a clear WARNING is printed so a CPU-only stack is never handed over silently. The runtime `install_torch()` above can repair this case from the Detect Hardware modal without a full rebuild.

The launcher's [`_has_nvidia_gpu()`](../../../veritate.py#L420) mirrors `deps.has_nvidia_gpu()`: same nvidia-smi + `nvcuda`/WMI (Windows) + `/dev/nvidia0` / `libcuda.so.1` / DRM vendor id (Linux) cross-check. macOS is untouched: it rides PyPI (arm64 to MPS, Intel to CPU).
...
- **`--user` on a system Python can put the install on a `PYTHONPATH` the running dashboard doesn't include.** The retry works because the Auto tune modal re-invokes `/trainers/run`, which spawns a fresh trainer subprocess that re-reads the `--user` site directory. In-process code that fails to import after `install(--user)` still needs a dashboard restart because `sys.path` is only computed at interpreter start.
...
- **The UAC prompt on Windows is asynchronous.** `ShellExecuteW` returns as soon as it dispatches; success/failure of the actual install is observed by the pipe on the elevated process, which this shim does not capture. The code only knows whether the launch itself succeeded (`rc > 32`). If pip fails inside the elevated shell, the client sees `ok: True`, and the modal's retry re-hits the ImportError, which is the honest signal.
