...
- Device comes from `veritate_core.plugin.hardware.pick_device()`; batches come from its own `make_loader` (seeded memmap slicing, same contract as the trainer's).
- Corpus stem resolves through `save.resolve_corpus`, which covers both shared (`data/corpus/`) and bundled stems.
