...
| [tools/](../../../veritate_mri/tools/)                                 | Corpus builders invoked by the corpus routes                                 |
| [teacher/](../../../veritate_mri/teacher/)                             | Teacher (Ollama / API) integration                                           |
| [data/](../../../veritate_mri/data/)                                   | Machine-local state and shipped data: settings, eval sets, platform KB, and the in-app wiki under [data/wiki/](../../../veritate_mri/data/wiki/) |
...
5. Route registration (22 modules under `routes/`).
6. Mesh setup (conditional on `mesh_role`).
7. App-sync reload hook bind (`app_sync.set_reload_hook`).
8. Optional eager brain load on background thread.
9. Optional C-engine warm-pool spawn for `settings.warm_models`.
