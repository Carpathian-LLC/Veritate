...
The `learning` tab is labeled "Models" in the bar; `generation` carries `active` in the markup, so it is the tab shown on load.

Tab bodies use the same `data-tab` attribute; CSS rules at [index.css:58-59](../../../veritate_mri/web/index.css#L58) show only the body whose data-tab matches the active class. The chat body is the one exception: it is `display: flex` when active so the embedded chat page fills the viewport.
...
- Each tab's per-tab init function (`startTrainPolling`, `ensureLearningLoaded`, `ensureWikiLoaded`, the `chat_tab.js` init, and so on).

## Pitfalls

- The `valid` array is the sole source of truth for valid tab names. Don't rely on `data-tab` to advertise routability.
- Hash navigation doesn't push to history if the same tab is clicked twice. Listen to both `hashchange` and `click` for an event on every activation.
