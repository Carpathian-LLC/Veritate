# models.html: Trading model-analytics page (experimental)

Vanilla-JS + canvas comparison ground served at `/ext/trading/models`
(`extensions/canonical/trading/page/models.html`, an extra page route added by the Trading
extension's `register.py`). No build step, no chart library, no framework: it only calls
`/ext/trading/market/*` on the canonical server. The main `/ext/trading` page links to it
from the Research tab ("Open model analytics"). 100 percent ASCII copy (no emojis, no
emdashes).
...
The backend already threads `source` end to end: `/ext/trading/market/instruments`, `/ext/trading/market/veritate_hindcast`,
`/ext/trading/market/veritate_benchmark`, `/ext/trading/market/veritate_data_report`, and `/ext/trading/market/veritate_live` all take
a `source` arg and pass it to `data.list_instruments(source)` / `data.load_tail(..., source=...)`
(`market/data.py`). Stock CSVs live at `extensions/installed/trading/data/extension_data/stocks/` (503 daily-OHLCV tickers, schema
...
## Data flow
`loadModels()` fills the single compare picker from `/ext/trading/market/veritate_models` (no slice, no type
filter) and sets the `#primaryLbl` line; `loadInstruments()` fills the symbol typeahead's `<datalist>`
options from `/ext/trading/market/instruments` (local cached symbols unioned with the fetchable crypto
...
clamped to [300, 20000]). **Run** fans out `/ext/trading/market/veritate_hindcast` for each **checked** model
(the overlay) and `/ext/trading/market/veritate_benchmark` for the primary (first-checked) model (metrics, trading,
...
`/ext/trading/market/veritate_hindcast` across the majors for the grid. The **Data inventory** view
lazy-fetches `/ext/trading/market/veritate_data_report`. **Live** toggles a 12s `pollLive()` loop on
`/ext/trading/market/veritate_live`, appending the latest close and redrawing the next-bar ray.
