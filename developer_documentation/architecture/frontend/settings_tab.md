...
- The **Extensions** section inside the Corpus library card (`#extBrowseBtn`, popup `#extensionsModal`, `_ext*` in `index.js`) lists downloadable add-on market datasets from `GET /market/extensions/catalog`, mirroring the corpus-library UI. Each row shows size + local status with a **download** button (placeholder/`coming soon` until the catalog url lands) and a **delete** button (reclaims disk; symlinked datasets only lose the link, and a not-yet-hosted dataset confirms that delete is permanent). Datasets live under `external_data/extension_data/<source>`. See [../backend/market_extensions.md](../backend/market_extensions.md).
