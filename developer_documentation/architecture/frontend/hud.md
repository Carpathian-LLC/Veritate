...
- Fed by periodic `GET /sys_metrics`, which returns `sys_metrics.snapshot()` from [sys_metrics.py](../../../veritate_mri/runtime/sys_metrics.py). Field-by-field shape and per-OS sources are in [hud_metrics.md](hud_metrics.md).
...
- Routes in [sys_routes.py](../../../veritate_mri/routes/sys_routes.py): `/sys_metrics` (the poll), `/sys/specs` and `/sys/detect` (hardware detection), `/sys/mode` and `/sys/mode/relaunch` (power-save mode).
