...
- Wired per run via reserved flags `slm_ref` (model dir name of the reference run; empty = off) and `slm_keep` (default 0.6) in `veritate_trainer.py`. Applied to the TRAINING loss only; validation stays unmasked so val bpb remains comparable across arms.
