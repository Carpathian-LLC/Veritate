...
`RecurrentMixer(hidden, heads, state_rule=...)` selects how the per-head state is updated. Threaded through `RecurrentBlock`, `VeritateRecurrent`, and `VeritatePatched` (the hybrid trunk's global mixer), and exposed to training via the `state_rule` reserved flag (`vanilla_trainer.py::RESERVED_STR_FLAGS`, default `gla`, injected into the model only for `trunk` in {`hybrid`, `hybrid_moe`, `recurrent`} and only when non-default). Long-context memory mechanisms M1 (`delta`) and M2 (`pinned`).
