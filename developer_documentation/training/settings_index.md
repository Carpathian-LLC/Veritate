...
- **model_type**: `language` | `code` | `statistical` | `other`. Gates the `LANGUAGE_DUMPS` set in `veritate_mri/training/save.py` (grades, reading_comprehension, math, grammar, reasoning, concepts, writing_health, generation) and, separately, the eval-deep suite (MMLU, HellaSwag, IFEval) gated in `veritate_mri/routes/runs_routes.py`. `language` and `code` both get both suites; the runtime gate is `NON_LANGUAGE_TYPES = frozenset({"statistical", "other"})`, so only those two skip them (rule 24a).
...
The fastest sensible config for a genuinely conversational byte model **given a chat corpus of `chat_v1`+`chat_v2`+`chat_v3` = 4.9 GB**. Do NOT run this against a mostly-code corpus; the model will not talk.
...
**Speed levers applied on this box**: pinned+prefetch dataloader, RAM-resident corpus, fused AdamW (auto when `optimizer=muon` falls back to AdamW for 1-D params), `F.rms_norm`, in-forward L1. Measured ceiling ~60k tok/s on this 5070.

**What "perfect" cannot mean here.** The measured wall is honest: one 5070 + 85 M params + 4.9 GB chat corpus produces a small, fluent chatbot, not GPT-4. Closer to that ceiling, the next lever is size (200 M scale on this same recipe once the 85 M converses), not more knobs on 85 M.

**Pushback / rule 9a.** A mixed-code corpus under this config produces a coder that stumbles at chat, not "insanely fluent chat". Confirm the corpus above (or an equivalent chat-dominant mix) exists on disk before launching, or the 4-8 h of compute is wasted.
