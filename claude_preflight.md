...
24a. **Training launches go through the dashboard `/trainers/run` endpoint on the live server (never a one-off launcher), and the request body MUST set `model_type` to match the model's purpose:** `language` (default) | `code` | `statistical` | `other`. Trainers drop `--model_type` (`parse_known_args`), and `trainer_runner` exports `VERITATE_MODEL_TYPE` only when the arg is present, so an omitted `model_type` silently defaults to `language` — wrong hook gating (the `LANGUAGE_DUMPS` set in `veritate_mri/training/save.py` accrues meaningless reading/grammar/math scores on a non-text model) and a wrong eval-deep gate (`runs_routes.py` refuses language benchmarks for `code`/`statistical`/`other`). A Python/coding model is `code`. To correct a run already launched without it, set `training_args.model_type` in the model's `config.json` — the save path reads it fresh at the next checkpoint and `_ensure_config` won't clobber an existing file.
