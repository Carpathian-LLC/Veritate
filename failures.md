...
**Raising the learning rate does not replace samples for full-parameter AdamW at cardinal-sized doses (IDEA 20 E4 arms 1-3, 2026-09-02)**
exp_fastsleep_0902 (wren1_0@1250), fact_sft:0.75/mixed_chat:0.25, batch 7, 20-60 steps, engine quiz. AdamW 1e-4: veritate_chat +65.1% at step 20, recall 0/50 fwd 1/50 rev (killed). 3e-5: +10.6% at 20, 1/1 of 50 (killed by the +5% line, which turned out to be a transient). 1e-5 to 60 steps: 0/0, 2/0, 3/3 of 50 with mixed_chat +14.9/+5.5/+5.2% on the 8-draw windows. Kill line: no full-parameter AdamW arm at ≤60 steps (≤1.7 MB) reached 10/100; the integrated-lr equivalence with E4's Muon 5e-6 does not carry across optimizers (fresh AdamW moments make ~lr-sized sign updates on every weight). Retry condition: none for the lr lever alone; the successor (freeze the lower 15 blocks, 3e-5, 200+ steps) is in successes.md. Lab: 2026-09-02-fast-consolidation-on-cardinal.
...
Loss on corpora the sleep runs never touched, measured through the trainer's own `evaluate()`,
8 iters, seed 1234. On `mixed_chat_val.bin`: **step 0 0.572240, step 10 0.574835 (+0.45%), step 12
0.575393 (+0.55%)**, monotone with dose. On `veritate_chat_val.bin`, closer to the model's actual
chat task: **step 0 0.996948, step 12 1.011243 (+1.43%)** -- the damage is nearly 3x larger measured
against the distribution the model is actually for, which is the number that matters. The model's own experience
...
**The first val row after a resume is spurious, and `stop_on_val_rise` would kill a healthy run on it (2026-09-17)**
`exp_chatsft_0913` resumed at step 148,000 and its first eval, at 148,250, reported **0.609826** against a pre-pause band of 0.3495-0.3503 held across thirteen readings - a +74% jump. The weights were fine. Scored independently through `tools.val_eval` in ONE process with ONE loader, the two checkpoints are **0.36165 (148,000) vs 0.36551 (148,250), +1.1%**; the trainer's own NEXT row, at 148,500, reads **0.355011**, back in band. Train loss, grad norm, throughput and the launch banner were unchanged across the resume - the val row was the only number that moved, which is not the shape a real regression takes.
Kill line for anyone reading a resumed run: **discard the first val row after any resume**; it is not comparable to the rows before it. The danger is not the false alarm, it is `stop_on_val_rise`, which arms on exactly this comparison - a run resumed with the rule armed checkpoints and stops at its first eval on an artifact. The rule already requires two consecutive readings, which is what would have saved it here; the one-reading version would not have. Lab: 2026-09-13-wren2-chat-sft-and-scale-benchmark.
...
**Code-QA consolidation stores without retrieving: generation null against a 3.2x likelihood gain (IDEA 20 E4 generalization, wren1_12, 2026-08-26)**
Clean fork of wren1_0@1250, `codeqa:0.75,mixed_chat:0.25` (48 defs, 20 QA forms both directions), lr 5e-6, self-limited at step 150 (~35 epochs). Likelihood recall monotone at every checkpoint: studied NLL 1.7804→0.5524, gap vs held-out −0.05→+0.24. Generation (containment, 25/25): **studied 1/25, held-out 2/25** — the control won. Arbitrary paired associates (name→file) never bind; semantically supported ones (args, docstrings) do. Kill line: memorized-but-unextractable is the failure mode for document study on a 200M byte model at this dose; further storage levers (epochs, lr, forms) are aimed at the solved half. Full record: `lab/2026-08-25-e4-generalization-run.md`.
